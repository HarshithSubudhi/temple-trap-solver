...
class GameVisualizer:
    ...
    def draw_state(self, gs):
        ...
        for idx, tile_symbol in enumerate(gs.tiles):
            ...
            # orientations are stored as counter-clockwise rotational factors (0=0, 1=90, 2=180, 3=270)
                ...
                if event.type == pygame.KEYDOWN:
                    if event.key in [pygame.K_SPACE, pygame.K_RIGHT]:
                        if step_idx < len(solution_path):
                            ...
                            if action == "slide":
                                current_state = current_state.slide(arg)
                            elif action == "walk":
                                dest_idx, dest_layer, _ = arg
                                current_state = GameState(current_state.tiles, current_state.rotations, dest_idx, dest_layer)
                            step_idx += 1
                            self.draw_state(current_state)
                        else:
