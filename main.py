...
def run_level(level_name):
    ...
    # Instantiate GameState matching the signature: (tiles, rotations, pawn, layer)
    solver_state = GameState(lvl['board'].copy(), lvl['orientation'].copy(), lvl['pawn_pos'], "Ground")
    vis_state = GameState(lvl['board'].copy(), lvl['orientation'].copy(), lvl['pawn_pos'], "Ground")

    print(f"\nProcessing structural map: {level_name}...")
    t0 = time.perf_counter()
    solution_path, total_cost = astar_solver(solver_state)
    t1 = time.perf_counter()
    
    solve_time_ms = (t1 - t0) * 1000
    print(f"A* Search completed in {solve_time_ms:.2f} ms")
